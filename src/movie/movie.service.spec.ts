...
describe('MovieService', () => {
	...
	describe('findAll', () => {
		...
		it('should not call getLikedMovies if a list of movies is empty', async () => {
			const movies = [];
			...
			expect(getMoviesMock).toHaveBeenCalled();
			expect(getLikedMoviesMock).not.toHaveBeenCalled(); // getLikedMovies가 호출되지 않아야 함
			expect(result).toEqual({
				data: [],
				nextCursor: null,
				count: 0,
		...
		it('should return a list of moives with user likes', async () => {
			const movies = [
				{
					id: 1,
					title: 'Movie 1',
	...
	describe('findOne', () => {
		...
		beforeEach(() => {
			findMovieDetailMock = jest.spyOn(movieService, 'findMovieDetailById');
		});
	...
	describe('update', () => {
		...
		it('should update a movie successfully', async () => {
			...
			const movie = { id: 1, detail: { id: 1 }, genres: [{ id: 1 }, { id: 2 }] } as Movie;
			const director = { id: 1, name: 'Director' } as Director;
			const genres = [
				{ id: 1, name: 'Genre 1' },
				{ id: 2, name: 'Genre 2' },
	...
	describe('toggleMovieLike', () => {
		...
		it('should throw BadRequestException if movie does not exist', async () => {
			...
			await expect(movieService.toggleMovieLike(1, 1, true)).rejects.toThrow(NotFoundException);
