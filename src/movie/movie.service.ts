...
export class MovieService {
    ...
    async findAll(dto: GetMoviesDto, userId?: number) {
        ...
        const hasNext = movies.length > take;

        if(hasNext) movies.pop();

        const nextCursor = hasNext ? movies[movies.length -1 ].id.toString() : null;
        ...
        return { data: movies, nextCursor, hasNext };
    }
