...
export class MovieService {
	...
	/* istanbul ignore next */
	async createMovie(qr: QueryRunner, createMovieDto: CreateMovieDto, movieDetailId: number, director: Director, userId: number, genres: Genre[], movieFolder: string) {
		// return await qr.manager
		//     .createQueryBuilder()
		//     .insert()
		//     .into(Movie)
	...
	async renameMovieFile(tempFolder: string, movieFolder: string, createMovieDto: CreateMovieDto) {
		...
		if (process.env.ENV !== 'prod') {
			return await rename(join(process.cwd(), tempFolder, createMovieDto.movieFileName), join(process.cwd(), movieFolder, createMovieDto.movieFileName));
		} else {
